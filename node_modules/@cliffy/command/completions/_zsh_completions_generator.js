...
/** Generates zsh completions script. */ export class ZshCompletionsGenerator {
  ...
  cmd;
  actions;
  /** Generates zsh completions script for given command. */ static generate(name, cmd) {
...
if [ "\${funcstack[1]}" = "_${this.name}" ]; then
  ...
  /** Generates zsh completions method for given command and child commands. */ generateCompletions(name, command, path = "") {
    if (!command.hasCommands(false) && !command.hasOptions(false) && !command.hasArguments()) {
      return "";
    }
    path = (path ? path + " " : "") + name;
    return `# shellcheck disable=SC2154
` + (command.getParent() ? `(( $+functions[_${replaceSpecialChars(path)}] )) || ` : "") + `_${replaceSpecialChars(path)}() {` + (!command.getParent() ? `
  ...
  generateCommandCompletions(command, path) {
    ...
    let completions = commands.map((subCommand)=>`'${subCommand.getName().replace(/:/g, "\\:")}:${subCommand.getShortDescription()// escape single quotes
      .replace(/'/g, "'\"'\"'")}'`).join("\n      ");
    if (completions) {
      completions = `
    local -a commands
  ...
  generateActions(command) {
    ...
    if (this.actions.size) {
      actions = Array.from(this.actions).map(([name, action])=>`${name}) __${replaceSpecialChars(this.name)}_complete ${action.arg.name} ${action.arg.action} ${action.cmd} ;;`);
    }
    if (command.hasCommands(false)) {
...
//# sourceMappingURL=_zsh_completions_generator.js.map