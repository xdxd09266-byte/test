...
import type { SecretType } from "./types/secret.js";
...
export type TypedArguments<TArgs extends string, TTypes> = number extends TTypes ? any : TArgs extends `${infer TArg} ${infer TRestArgs}` ? TArg extends `[${string}]` ? [ArgumentType<TArg, TTypes>?, ...TypedArguments<TRestArgs, TTypes>] : [ArgumentType<TArg, TTypes>, ...TypedArguments<TRestArgs, TTypes>] : TArgs extends `${string}...${string}` ? [...ArgumentType<TArgs, TTypes> extends ReadonlyArray<infer TValue> ? TArgs extends `[${string}]` ? Array<TValue> : [TValue, ...Array<TValue>] : never] : TArgs extends `[${string}]` ? [ArgumentType<TArgs, TTypes>?] : [ArgumentType<TArgs, TTypes>];
export type TypedArgument<TArg extends string, TTypes, TDefault = undefined> = number extends TTypes ? any : TArg extends `${string}...${string}` ? ArgumentType<TArg, TTypes> extends ReadonlyArray<infer TValue> ? TArg extends `[${string}]` ? Array<TValue> | MakeArray<NonNullable<TDefault>> : [TValue, ...Array<TValue>] | MakeArray<NonNullable<TDefault>> : never : TArg extends `[${string}]` ? IsRequired<TDefault extends undefined ? false : true, TDefault> extends true ? [NonNullable<TDefault> | ArgumentType<TArg, TTypes>] : [(TDefault | ArgumentType<TArg, TTypes>)?] : [NonNullable<TDefault> | ArgumentType<TArg, TTypes>];
export type TypedArgumentValue<TArg extends string, TTypes, TDefault = undefined> = number extends TTypes ? any : TArg extends `${string}...${string}` ? ArgumentType<TArg, TTypes> extends ReadonlyArray<infer TValue> ? TArg extends `[${string}]` ? NonNullable<TDefault> | Array<TValue> : NonNullable<TDefault> | [TValue, ...Array<TValue>] : never : NonNullable<TDefault> | ArgumentType<TArg, TTypes>;
...
export type MapValue<TOptions, TMappedOptions, TCollect = undefined> = TMappedOptions extends undefined ? TCollect extends true ? {
  [Key in keyof TOptions]: TOptions[Key] extends (Record<string, unknown> | undefined) ? MapValue<TOptions[Key], TMappedOptions, TCollect> : string extends Key ? Array<NonNullable<TOptions[Key]>> | undefined : Array<NonNullable<TOptions[Key]>>;
} : {
  [Key in keyof TOptions]: TOptions[Key] extends (Record<string, unknown> | undefined) ? MapValue<TOptions[Key], TMappedOptions, TCollect> : string extends Key ? TOptions[Key] | undefined : TOptions[Key];
} : {
  [Key in keyof TOptions]: TOptions[Key] extends (Record<string, unknown> | undefined) ? MapValue<TOptions[Key], TMappedOptions, TCollect> : TMappedOptions;
};
export type MapTypes<T> = T extends TypeOrTypeHandler<infer V> ? V : T extends Record<string, unknown> | ReadonlyArray<unknown> ? {
...
export type IsRequired<TRequired extends boolean | undefined, TDefault> = TRequired extends true ? true : TDefault extends undefined ? false : true;
type MakeArray<TValue> = TValue extends ReadonlyArray<unknown> ? TValue : [TValue];
