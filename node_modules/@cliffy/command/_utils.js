import { UnexpectedArgumentAfterVariadicArgumentError, UnexpectedRequiredArgumentError } from "@jsr/cliffy__flags";
...
export function parseArgumentsDefinition(argsDefinition, validate = true, all) {
  ...
  for (let argDef of parts){
    ...
    const parts = argDef.arg.split(ARGUMENT_DETAILS_REGEX);
    if (!parts[1]) {
      if (all) {
