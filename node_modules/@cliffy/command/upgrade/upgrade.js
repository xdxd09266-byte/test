import { bold, brightBlue, dim, red } from "@jsr/std__fmt/colors";
 ...
 */ export async function upgrade({ runtime: runtimeOptions, provider, ...options }) {
  if (options.force || !options.from || await provider.isOutdated(options.name, options.from, options.to)) {
    if (options.to === "latest") {
      options.logger?.log(dim("Upgrading %s to the %s version"), options.name, options.to);
    } else {
      options.logger?.log(dim("Upgrading %s to version %s"), options.name, options.to);
    }
    ...
    try {
      ...
      options.logger?.error(red(`Failed to upgrade ${bold(options.name)} ${options.from ? `from version ${bold(options.from)} ` : ""}to ${bold(options.to)}.`));
      throw error;
    }
    options.logger?.info(brightBlue(`Successfully upgraded ${bold(options.name)} from version ${bold(options.from ?? "")} to ${bold(options.to)}!`), dim(`(${provider.getRepositoryUrl(options.name, options.to)})`));
  }
}
