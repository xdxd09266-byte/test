import { bold, brightBlue } from "@jsr/std__fmt/colors";
 ...
 */ export class UpgradeCommand extends Command {
  ...
  constructor({ provider, spinner: withSpinner = true, ...options }){
    ...
    this.description(()=>`Upgrade ${this.getMainCommand().getName()} executable to latest or given version.`).noGlobals().type("provider", new EnumType(this.getProviderNames())).option("-r, --registry <name:provider>", `The registry name from which to upgrade.`, {
      ...
      default: "latest"
    }).option("-f, --force", "Replace current installation even if not out-of-date.").option("-v, --verbose", "Log verbose output.").option("--no-spinner", "Disable spinner.", {
      hidden: !withSpinner
    }).complete("version", ()=>this.getAllVersions()).action(async ({ registry: provider, version, force, verbose, spinner: spinnerEnabled })=>{
      const name = this.getMainCommand().getName();
      const currentVersion = this.getVersion();
      const spinner = withSpinner && spinnerEnabled ? new Spinner({
...
//# sourceMappingURL=upgrade_command.js.map