 ...
 * ```
 */ export const ASCII_DIACRITICS_REGEXP = /(?<=[a-zA-Z])\p{M}+|[^\p{L}\p{M}\p{N}\-]+/gu;
/**
 ...
 */ export function slugify(input, options) {
  ...
  const words = [];
  for (const s of wordSegmenter.segment(input.trim().normalize("NFD").toLowerCase())){
    if (s.isWordLike) {
  ...
  return words.map(options?.transliterate ?? ((x)=>x)).join(options?.transliterate ? "-" : "").replaceAll(stripRe, "").normalize("NFC").replaceAll(/-{2,}/g, "-").replaceAll(/^-|-$/g, "") || "-";
}
//# sourceMappingURL=unstable_slugify.js.map