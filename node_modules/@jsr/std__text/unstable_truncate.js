 ...
 */ export function truncate(str, maxLength, options) {
  if (!Number.isInteger(maxLength) || maxLength < 0) {
    throw new RangeError(`Cannot truncate: maxLength must be a non-negative integer, received ${maxLength}`);
  }
    ...
    case "middle":
      {
        ...
        return str.slice(0, adjustSplitBack(str, leftLen)) + suffix + str.slice(adjustSplitForward(str, rightStart));
      }
    case "end":
      return str.slice(0, adjustSplitBack(str, budget)) + suffix;
    default:
      throw new TypeError(`Cannot truncate: position must be "end", "middle", or "start", received "${position}"`);
  }
}
/**
 * If `index` lands on a low surrogate, back up one position so we don't split
 * a surrogate pair.
 */ function adjustSplitBack(str, index) {
 ...
 * a surrogate pair.
 */ function adjustSplitForward(str, index) {
  if (index > 0 && index < str.length) {
    const code = str.charCodeAt(index);
    if (code >= 0xDC00 && code <= 0xDFFF) return index + 1;
  }
...
//# sourceMappingURL=unstable_truncate.js.map