...
import { escape } from "@jsr/std__regexp/escape";
/**
 ...
 * ```
 */ export function trimBy(str, pattern) {
 ...
 * ```
 */ export function trimStartBy(str, pattern) {
 ...
 * assertEquals(result, "  Hello, World!");
 * ```
 */ export function trimEndBy(str, pattern) {
  return trimUntilDone(str, regExpFromTrimPattern`${pattern}$`);
}
function trimUntilDone(str, regex) {
  let current = str;
  let next;
  while((next = current.replace(regex, "")) !== current)current = next;
  return current;
}
function regExpFromTrimPattern(t, pattern) {
  let { source, flags } = pattern instanceof RegExp ? pattern : {
    source: `${typeof pattern === "string" ? escape(pattern) : [
      ...pattern
...
//# sourceMappingURL=unstable_trim_by.js.map