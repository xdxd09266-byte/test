...
import { titleCaseMapping } from "./_title_case_mapping.js";
...
export function resolveOptions(opts) {
  const { trailingCase, locale } = {
    ...defaultTitleCaseOptions,
    ...opts
  };
...
function resolveSerializableOptions(o) {
  ...
  const vals = {
    ...
    words: new Intl.Segmenter(segmenterLocale, {
      granularity: "word"
    }),
    graphemes: new Intl.Segmenter(segmenterLocale, {
...
function resolveLocaleOption(locale) {
  return locale === false ? "und" : locale === true ? defaultLocale() : Array.isArray(locale) ? locale[0]?.toString() ?? defaultLocale() : locale.toString();
}
// https://tc39.es/ecma402/#sec-defaultlocale
function defaultLocale() {
  return (globalThis.navigator?.language ?? // Bun [currently doesn't support `navigator.language`](https://github.com/oven-sh/bun/issues/3168),
  // so we read from `NumberFormat#resolvedOptions` as a backup.
  new Intl.NumberFormat().resolvedOptions().locale)// Per spec, can't contain a `-u-` extension sequence (Deno's default `navigator.language` is `en-US-u-va-posix`).
  .replace(/-u-.+/, "");
}
...
const TARGET_CHAR_REGEXP = // TODO(lionel-rowe): remove deno-lint-ignore comment once https://github.com/denoland/deno_lint/issues/1442 fixed
...
function _titleCaseSegment(s, opts) {
  ...
  if (opts.locale.language === "nl") {
    ...
    if (m != null) {
      const [ij] = m;
      return head + ij.toLocaleUpperCase(opts.locale) + (opts.trailingCase === "lower" ? tail.toLocaleLowerCase(opts.locale) : tail)// Invariant: The length of the "ij" pair remains the same in its title/uppercased vs lowercased forms.
      .slice(ij.length);
    }
  }
  const first = titleCaseGrapheme(match.segment, opts.locale);
  if (s.length === match.index + match.segment.length) return head + first;
  const lc = opts.trailingCase === "lower" ? tail.toLocaleLowerCase(opts.locale) : tail;
...
function titleCaseGrapheme(grapheme, locale) {
  const [char] = grapheme;
  return titleCaseChar(char, locale) + // Invariant: Lower-casing of grapheme tail never changes depending on surrounding chars (true as of Unicode 16.0.0).
...
//# sourceMappingURL=_title_case_util.js.map